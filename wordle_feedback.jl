...
function make_feedback_matrix(word_list)
...
const absurdlelookup = Dict{BitVector, Vector{NamedTuple{(:word, :feedback, :groupsize), Tuple{String, Int64, Int64}}}}()

# ╔═╡ 87f20038-b40a-4b5c-a879-17ebe1d5a6e9
const lookuptimes = Dict{BitVector, DateTime}()
...
function monitor_absurdle_results()
	while true
		handle_results()
	end
end
...
const absurdle_solutions = Set{NamedTuple{(:numsteps, :finalguess, :gamesteps), Tuple{Int64, NamedTuple{(:word, :feedback, :groupsize), Tuple{String, Int64, Int64}}, Vector{Tuple{String, Int64}}}}}()

# ╔═╡ b3090db1-ab5b-4b38-84bb-127ebebce523
const solution_channel = Channel{NamedTuple{(:numsteps, :finalguess, :gamesteps), Tuple{Int64, NamedTuple{(:word, :feedback, :groupsize), Tuple{String, Int64, Int64}}, Vector{Tuple{String, Int64}}}}}(Inf)
...
function process_solutions(input)
	...
	if !isempty(absurdle_solutions)
		clean_solutions!()
		solutionlist = sort(collect(absurdle_solutions), by = a -> a.gamesteps)
	else
		nothing
	end
end

# ╔═╡ 53ac5cee-73c4-45b8-82df-222cbd444209
displaysolutions(::Nothing, indstart, maxnum; kwargs...) = md"""#### Waiting for first results"""
...
const feedback_sets = make_feedback_sets(feedback_matrix)
...
function eval_absurdle_group(guess_index::Integer, possible_indices::AbstractVector, feedback_counts)
	...
	(groupsize, max_feedback_index) = findmax(feedback_counts)
...
function eval_absurdle_guesses(possible_indices::AbstractVector)
	...
	if groupsize == 1
		...
		results = [(word = nyt_valid_words[j], feedback = 242, groupsize = 1)]
...
const base_absurdle_guesses = sort(eval_absurdle_guesses(baseinds), by = a -> a.groupsize)

# ╔═╡ 9e6dd99a-0300-48ba-a296-b13aa761c1ea
function eval_absurdle_guesses_gamesteps(gamesteps) 
...
function solve_absurdle(gamesteps::Vector{Tuple{String, Int64}}; branchnum = 2, initbranch = 2)
	...
	guesses = eval_absurdle_guesses_gamesteps(gamesteps)
	...
	if length(guesses) == 1 
		...
		put!(solution_channel, (numsteps = numsteps, finalguess = first(guesses)[(:word, :feedback, :groupsize)], gamesteps = gamesteps))
		return true
	end
	...
	guesslist = guesses[1:num]

	if smallestgroup == 1 && groupnum > 1
		filter!(a -> a.feedback != 242, guesslist) 
	end
	...
	# for guess in guesslist
		solve_absurdle(vcat(gamesteps, (guess.word, guess.feedback)); branchnum=branchnum)
	end
...
md"""
Branching Factor: $(@bind branchnum NumberField(1:100, default = 2))
Initial Branching: $(@bind initbranch NumberField(1:100, default = 2))
"""
...
begin
	...
	md"""
	Click to run absurdle solver with branching factor of $branchnum and initial branch of $initbranch
...
if runabsurdle > 0
	solve_absurdle(Vector{Tuple{String, Int64}}(); branchnum = branchnum, initbranch = initbranch)
	Base.GC.gc()
...
md"""
Starting Index: $(@bind indstart Slider(1:100, show_value=true))

Max Number: $(@bind maxnum confirm(NumberField(1:100, default = 20)))
"""

# ╔═╡ 197d66ea-2185-4f92-a20c-ecd44972daad
if !isnothing(solutionlist)
	md"""
	Starting Word: $(@bind startingwords MultiCheckBox(unique([a.gamesteps[1][1] for a in solutionlist]), default = [first(solutionlist).gamesteps[1][1]]))
	"""
else
...
# ╔═╡ 5672b0f7-00fd-4a5e-b3d6-5a9ee69e725a
 function displaysolutions(solutionlist, indstart, maxnum; startingwords = Set([first(solutionlist).gamesteps[1][1]]))
	absurdledisplay = [show_wordle_game([a.gamesteps; (a.finalguess.word, a.finalguess.feedback)], 	show_overlay=false, sizepct = 25) for a in filter(a -> in(a.gamesteps[1][1], startingwords), solutionlist)]
	l = length(absurdledisplay)
	@htl("""
	<div class="absurdledisplay">
	$(absurdledisplay[min(indstart, l):min(indstart+maxnum-1, l)])
	</div>
	<style>
		.absurdledisplay {
			display: flex;
...
displaysolutions(solutionlist, indstart, maxnum; startingwords = isnothing(startingwords) ? startingwords : Set(startingwords))
