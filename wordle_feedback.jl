...
begin
	using PlutoUI, Random, HypertextLiteral, StaticArrays, Symbolics, SymbolicRegression, SymbolicUtils, Latexify, Arrow, DataFrames, PlutoProfile, BenchmarkTools, LinearAlgebra, Base.Threads, Dates
	TableOfContents()
end
...
const absurdlelookup = Dict{Tuple{Int64, BitVector}, Vector{NamedTuple{(:word, :feedback, :groupsize, :possible_indices), Tuple{String, Int64, Int64, BitVector}}}}()
...
function load_absurdle_lookup(;fname="absurdlelookup.bin")
	...
	convertkey(k) = (k[1], BitVector(k[2]))
	function convertval(v) 
		t = collect(v) |> first
		[(word = t.word, feedback = t.feedback, groupsize = t.groupsize, possible_indices = BitVector(t.possible_indices))]
	end
...
function queue_lookup()
	lookup = load_absurdle_lookup()
	put!(lookupqueue, lookup)
end
...
function eval_absurdle_group(guess_index::Integer, possible_indices::AbstractVector; feedback_counts = zeros(Int64, 243), hard_mode_indices = possible_indices)
	k = (guess_index, possible_indices)
	try
		absurdlelookup[k]
	catch
		feedback_counts .= 0 
		...
		new_possible_indices_list = [get_possible_indices(guess_index, feedback_index - 1) .* possible_indices for feedback_index in inds]
	
		result = [(word = nyt_valid_words[guess_index], feedback = inds[i] - 1, groupsize = groupsize, possible_indices = new_possible_indices) for (i, new_possible_indices) in enumerate(new_possible_indices_list)]
		put!(resultqueue, (k, result))
		return result
	end
end	

# ╔═╡ 6981ab37-a73a-4481-8c61-c1d43016a4f9
	function eval_absurdle_guesses(possible_indices::AbstractVector, hard_mode_indices::AbstractVector)
		# k = (possible_indices, hard_mode_indices, weights)
		# haskey(absurdlelookup, k) && return absurdlelookup[k]
		groupsize = dot(possible_indices, wordle_original_inds)
		groupsize == 1 && return [(word = first(nyt_valid_words[possible_indices .* wordle_original_inds]), feedback = 242, groupsize = 1, possible_indices = possible_indices)]
		guessinds = view(freqsortinds, 1:4000)
		feedback_counts = zeros(Int64, 243)
		results = mapreduce(i -> eval_absurdle_group(i, possible_indices; feedback_counts = feedback_counts, hard_mode_indices = hard_mode_indices), vcat, guessinds)
		# absurdlelookup[k] = results
		return results
	end

# ╔═╡ 78bc8334-de61-4262-9bba-d80dc11602f8
const base_absurdle_guesses = sort(eval_absurdle_guesses(baseinds, baseinds), by = a -> a.groupsize)

# ╔═╡ 9e6dd99a-0300-48ba-a296-b13aa761c1ea
function eval_absurdle_guesses_gamesteps(gamesteps; undosteps = Vector{Tuple{String, Vector{Int64}}}()) 
	...
	sort(eval_absurdle_guesses(answer_indices, hard_mode_indices), by = a -> a.groupsize)
end
...
function solve_absurdle(gamesteps::Vector{Tuple{String, Int64}}; branchnum = 2, initbranch = 2)
	...
	if length(guesses) == 1 
		...
		beststeps = put!(leaststeps, min(numsteps, take!(leaststeps)))
		@info "Best step so far of $beststeps.  Game $gamesteps ended with final guess of $(first(guesses).word) and possible remaining words of $(nyt_valid_words[first(guesses).possible_indices])"
		return [(numsteps = length(gamesteps), finalguess = first(guesses), gamesteps = gamesteps)]
	end
...
function eval_group(guess_index::Integer, possible_indices::AbstractVector, weights::Vector{Float64}; feedback_counts = zeros(Int64, 243), feedback_probabilities = zeros(243), hard_mode_indices = possible_indices)
	...
	s = sum(weights[possible_indices]) #this is faster even with allocations
	for (i, b) in enumerate(possible_indices)
		if b
			f = feedback_matrix[guess_index, i]
	...
	base_counts = all(hard_mode_indices)
...
begin
	...
	function eval_guesses(possible_indices::AbstractVector, weights::Vector{Float64}, hard_mode_indices::AbstractVector)
		k = hash((possible_indices, weights, hard_mode_indices))
...
begin
	...
	WordleGame(;answer_index_list=sample_random_answers(5000), nguesses = 6, showletters=true) = WordleGame(0, Vector{String}(), Vector{Vector{Int64}}(), answer_index_list, first(answer_index_list), nguesses, showletters, nyt_valid_words, :active)
	...
	Bonds.initial_value(input::WordleGame) = (guesses = Vector{String}(), feedback = Vector{Vector{Int64}}(), answerindex = input.answerindex, numguesses = input.numguesses, status = :active)
	Bonds.possible_values(input::WordleGame) = Bonds.InfinitePossibilities()
	Bonds.transform_value(input::WordleGame, val_from_js) = (guesses = Vector{String}(val_from_js[1]), feedback = Vector{Vector{Int64}}(val_from_js[2]), answerindex = val_from_js[3], numguesses = input.numguesses, status = Symbol(val_from_js[4]))
...
julia_version = "1.9.1"
manifest_format = "2.0"
project_hash = "e475016441a173d7ddcecd31f7cd4d7894284af0"
