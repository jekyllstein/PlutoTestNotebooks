...
begin
	using PlutoUI, Random, HypertextLiteral, StaticArrays, SymbolicUtils, Latexify, Arrow, DataFrames, PlutoProfile, BenchmarkTools, LinearAlgebra, Base.Threads, Dates, PlutoProfile, StatsBase, PlutoPlotly, SpecialFunctions
	TableOfContents()
end
...
Hover over cells to replay animation.  In the case of completed games hover can also reveal the answer word if the game was unsuccessful.
"""
...
Demonstrates input field that only accepts 5 character words which are sent to the `WordleGame` element to  to visualize the feedback.  You can see the letters populate as you type the guess and then get converted to feedback after the guess is submitted.
"""
...
# ╔═╡ f55d6e2c-5801-4985-8b28-49905143e0eb
HTML("""
<style>
	main {
...
function show_blank_squares(guess)
	...
	getlabel(i) = length(word) < i ? "" : "label = $(lowercase(word[i]))"
	
	function make_box(i)
		"""
		<div $(getclass(i)) $(getlabel(i))></div>
		"""
	end
	HTML("""
...
# ╔═╡ fbdbb245-0372-4058-b11f-d1d25afcc694
md"""
### Precomputed Vars
"""

# ╔═╡ 6dd2049b-3201-497b-ba48-04aff5506d95
...
md"""
Shows corresponding feedback from the matrix and the corresponding words they represent.  Confirms that the feedback calculation is done correctly.
"""

# ╔═╡ fb879e97-cece-407b-8d17-42cd73067731
...
# ╔═╡ 2daab9c0-af73-4708-82a3-7c72c17fc26b
md"""
### Absurdle Test
"""

# ╔═╡ 65a991b8-9bed-4dfe-82b0-503f5fa0bd17
const absurdlelookup = Dict{Tuple{BitVector, Vector{Int64}}, Vector{NamedTuple{(:word, :groupsize, :feedbacklist), Tuple{String, Int64, Vector{Int64}}}}}()

# ╔═╡ 87f20038-b40a-4b5c-a879-17ebe1d5a6e9
const lookuptimes = Dict{Tuple{BitVector, Vector{Int64}}, DateTime}()

# ╔═╡ 268ab0b8-f1f5-4801-b3bb-d31df4016e2a
const resultqueue = Channel{Tuple{Tuple{BitVector, Vector{Int64}}, Vector{NamedTuple{(:word, :groupsize, :feedbacklist), Tuple{String, Int64, Vector{Int64}}}}}}(Inf)

# ╔═╡ 805e96ec-c6db-4772-b1c0-5a13c96091d8
const lookup_time_channel = Channel{Tuple{Tuple{BitVector, Vector{Int64}}, DateTime}}(Inf)

# ╔═╡ 369b0e11-9412-4066-9373-7d2ba5d398c3
function handle_results(absurdlelookup, lookuptimes)
	t = take!(resultqueue)
	...
	if Sys.free_physical_memory()/Sys.total_physical_memory() < 0.2
		@info "Less than 20% of physical memory is free at $(now()).  Purging half of lookup dictionary"
		sortedkeys = [a[1] for a in sort(lookuptimes, by = a -> a[2])]
		l = length(sortedkeys)
		#remove the half of the keys that were accessed the longest ago
		for k in sortedkeys[1:round(Int64, l/2)]
	...
	# if Sys.free_physical_memory()/Sys.total_physical_memory() < 0.2
	# 	@info "Less than 20% of physical memory is free at $(now()).  Purging half of lookup dictionary"
	# 	for k in keys(absurdlelookup)
	# 		rand() > 0.5 && delete!(absurdlelookup, k)
	# 	end
	# 	GC.gc()
	# end
end
...
function monitor_absurdle_results(resultlookup, timelookup)
	while true
		handle_results(resultlookup, timelookup)
	end
end
...
begin 
	...
	const stepwatch = Channel{Int64}(Inf)
	put!(leaststeps, typemax(Int64))
end

# ╔═╡ 7520074c-2d51-4651-ac55-f5abe0a7d545
function watchsteps()
	while true
		minsteps = min(take!(stepwatch), take!(leaststeps))
		put!(leaststeps, minsteps)
	end
end

# ╔═╡ d4b57cce-c122-4fc5-98a6-b48ab11612a1
const progresschannel = Channel{Int64}(Inf)

# ╔═╡ 94212d55-b61a-45ce-94ca-66e13383dd59
function monitor_progress()
	numfinished = 0
	totalnum = 0
	start_time = now()
	while true
		num = take!(progresschannel)
		if num == -1
			start_time = now()
			@info "STARTING NEW SOLVER at $start_time"
		elseif num > 1
			totalnum = num
		elseif num == 1
			numfinished += 1
			@info "PROGRESS REPORT: $numfinished / $totalnum analyses finished in $((now() - start_time).value/1000) seconds"
		else
			numfinished = 0
			@info "Solution completed after $((now() - start_time).value/1000/60) minutes.  Waiting to check progress on new game"
			start_time = now()
		end
	end
end
...
const absurdle_solutions = Set{NamedTuple{(:numsteps, :finalguess, :gamesteps), Tuple{Int64, NamedTuple{(:word, :groupsize, :feedbacklist), Tuple{String, Int64, Vector{Int64}}}, Vector{Tuple{String, Int64}}}}}()

# ╔═╡ b3090db1-ab5b-4b38-84bb-127ebebce523
const solution_channel = Channel{NamedTuple{(:numsteps, :finalguess, :gamesteps), Tuple{Int64, NamedTuple{(:word, :groupsize, :feedbacklist), Tuple{String, Int64, Vector{Int64}}}, Vector{Tuple{String, Int64}}}}}(Inf)

# ╔═╡ 04b6f828-707d-41d9-a5ab-4c9d681e62df
function watch_solutions()
	while true
		newsolutions = Vector{eltype(solution_channel)}()
		push!(newsolutions, take!(solution_channel))
		while isready(solution_channel)
			push!(newsolutions, take!(solution_channel))
		end
		# @info "Adding $(length(newsolutions)) solutions"
		union!(absurdle_solutions, Set(newsolutions))
	end
end

# ╔═╡ a6503db6-a475-4912-9f96-07393262dcbb
#only keep solutions that are equal to the best seen so far
function clean_solutions!()
	minsteps = fetch(leaststeps)
	filter!(a -> a.numsteps <= minsteps, absurdle_solutions)
end
...
function load_absurdle_lookup(;fname="absurdlelookup.bin")
	fload = Arrow.Table(fname)
	convertkey(k) = BitVector(k)
	function convertval(v) 
		t = collect(v) |> first
		[(word = t.word, feedback = t.feedback, groupsize = t.groupsize)]
	end
	Dict(convertkey(a[1]) => convertval(a[2]) for a in zip(fload.key, fload.value))
end

# ╔═╡ ce65a46f-e92a-4a32-b8fd-c009c3654500
@async monitor_absurdle_results($absurdlelookup, $lookuptimes)

# ╔═╡ 4b64fdcd-a7d0-4f83-b97e-8c1fe300fa6c
@async watch_solutions()
...
# ╔═╡ 5537a8c5-ed23-4ef5-9ae8-e5ea465b84f7
const previousruns = Vector{Union{Nothing, NamedTuple{(:branchnum, :initbranch), Tuple{Int64, Int64}}}}()

# ╔═╡ e4284aa6-e759-49a6-890d-3c69574092af
push!(previousruns, nothing)
...
function process_solutions(input)
	#wait until the channel is empty after being filled
	while isready(solution_channel)
		sleep(0.1)
	end

	isempty(absurdle_solutions) && return nothing
	input == 0 && return sort(collect(absurdle_solutions), by = a -> a.gamesteps)
	
	
	clean_solutions!()
	sort(collect(absurdle_solutions), by = a -> a.gamesteps)
end

# ╔═╡ 5672b0f7-00fd-4a5e-b3d6-5a9ee69e725a
 function displaysolutions(absurdledisplay, solutionlist, indstart, maxnum, newdisplay; startingwords = Vector{String}())
	isempty(absurdledisplay) && return md"""#### Waiting for first results"""
	isempty(startingwords) && return md"""#### Select a starting word to display game solutions"""
	inds = findall(a -> in(a.gamesteps[1][1], startingwords), solutionlist)
	filtdisplay = view(absurdledisplay, inds)
	l = length(filtdisplay)
	@htl("""
	<div class="absurdledisplay">
	$(filtdisplay[min(indstart, l):min(indstart+maxnum-1, l)])
	</div>
	<style>
		.absurdledisplay {
			display: flex;
...
# ╔═╡ 8600afc0-cdb7-4163-a4eb-338682d964b3
const absurdledisplay = Vector{HTML{String}}()

# ╔═╡ 111a9c5d-6862-45d3-af1b-b3d641268d16
(absurdlelookup, length(absurdlelookup))

# ╔═╡ 69c4f6f5-bd94-4248-afc3-6ba7aad58194
(lookuptimes, length(lookuptimes))

# ╔═╡ 0b9befd8-6bba-4876-8d51-7695df9d80a7
(absurdle_solutions, length(absurdle_solutions))

# ╔═╡ 302f88a5-6cdc-402e-ac5f-df6a9a628091
Base.GC.gc()
...
# ╔═╡ 91506b6d-96e1-48d8-9768-2e6cc2c261b0
...
function get_possible_indices(gamesteps)
	isempty(gamesteps) && return baseinds
	length(gamesteps) == 1 && return get_possible_indices(first(gamesteps)...)
	mapreduce((a, b) -> a .* b, gamesteps) do s
		get_possible_indices(s...)
	end
end

# ╔═╡ 51c88ba2-a30b-40f9-949d-7e5253b073e8
const index_allocations = [deepcopy(baseinds) for _ in 1:nthreads()]

# ╔═╡ 296b4051-9875-4a91-980c-b972e44c297f
const index_channel = Channel{BitVector}(length(index_allocations))

# ╔═╡ b277b81f-a090-4fba-9026-41d6bd335c5b
for v in index_allocations put!(index_channel, v) end

# ╔═╡ 4e4166cd-2279-4636-a798-4e5a188b53ff
const feedback_allocations = [zeros(Int64, 243) for _ in eachindex(nyt_valid_words)]

# ╔═╡ 84f9697c-ec35-4153-892e-d3b948f8b59c
const feedback_count_channel = Channel{Vector{Int64}}(length(feedback_allocations))

# ╔═╡ 56861cf0-5d3e-4a57-bb76-07b57a17fd2f
for v in feedback_allocations put!(feedback_count_channel, v) end
...
function uniform_ncommon_sample(n = 3000)
	list = collect(keys(word_frequencies))[sortperm(values(word_frequencies) |> collect, rev=true)][1:n]
	rand(list)
end
...
# ╔═╡ c00ec17d-cbbb-4a80-b074-b231ccb95979
...
md"""
Actual and approximate cummulative distribution function for answers from the original wordle game as a function of the frequency rank.  It approximately follows a one sided normal distribution with more frequent words having a higher probability.  The derivative of the approximate curve can be used to sample new wordle answers based on their frequency rank to mimic the original distribution but allowing for differences.
"""
...
begin
	...
	t1 = scatter(x = wordranks, y = wordcdf, name = "Wordle Original CDF")
	t2 = scatter(x = wordranks, y = wordle_cdf.(wordranks), name = "Error Function Approximation")
	plot([t1, t2])
end

# ╔═╡ bf36fbee-94b8-499d-977e-ccbd5119e80d
plot(wordranks, wordle_pdf.(wordranks))

# ╔═╡ aea4f4ab-fb01-48da-be94-0b33ea443754
...
# ╔═╡ eeaf6088-5f00-4c83-a2df-aacb01ec0753
...
# ╔═╡ 18faf50a-51d4-4d45-ad78-1c43255ff6f2
...
# ╔═╡ 053d2bef-c4f8-497d-bb4e-f5cefa07ca89
md"""
# Option to Save Precomputed Data to Disk 

Two data structures are precomputed and kept in memory while the notebook is running.  Both of these can be saved to disk and loaded considerably faster than computing from scratch.  By default, they are not saved to disk and will take approximately 7GB of space.  If you would like to save these data structures click the button below.

$(@bind savedata CounterButton())
"""
...
function eval_absurdle_group(guess_index::Integer, possible_indices::AbstractVector, feedback_counts)
	view(feedback_counts, guess_index, :) .= 0 
	for (i, b) in enumerate(possible_indices)
		if b
			f = feedback_matrix[guess_index, i]
			#only count feedback for answers that were in the original wordle answer list
			feedback_counts[guess_index, f+1] += wordle_original_inds[i]
		end
	end

	#this is the feedback guaranteed to be received
	groupsize = maximum(view(feedback_counts, guess_index, :))
	inds = findall(a -> a == groupsize, feedback_counts)
	# if count(a -> a == groupsize, feedback_counts) > 1
	# 	@info "non unique worst group"
	# end
	# put!(feedback_count_channel, feedback_counts)
	[(word = nyt_valid_words[guess_index], feedback = i - 1, groupsize = groupsize) for i in inds]
end	

# ╔═╡ 4240eb52-a183-4297-9687-f8a03d5efe54
function eval_absurdle_groups(guess_indices::Vector{Int64}, possible_indices::BitVector, feedback_counts::Vector{Int64})
	results = Vector{NamedTuple{(:word, :groupsize, :feedbacklist), Tuple{String, Int64, Vector{Int64}}}}()
	for guess_index in guess_indices
		...
		for (i, b) in enumerate(possible_indices)
			if b
				f = feedback_matrix[guess_index, i]
				#only count feedback for answers that were in the original wordle answer list
				feedback_counts[f+1] += wordle_original_inds[i]
			end
		end
		groupsize = maximum(feedback_counts)
		feedbacklist = findall(a -> a == groupsize, feedback_counts) .- 1
		push!(results, (word = nyt_valid_words[guess_index], groupsize = groupsize, feedbacklist = feedbacklist))
	end
	return results
end

# ╔═╡ 6981ab37-a73a-4481-8c61-c1d43016a4f9
function eval_absurdle_guesses(possible_indices::BitVector, guessinds::Vector{Int64}, feedback_counts::Vector{Int64})
	k = (possible_indices, guessinds)
	...
	# 	put!(lookup_time_channel, (k, now()))
	# 	return absurdlelookup[k]
	# end
	groupsize = dot(possible_indices, wordle_original_inds)
	if groupsize == 1
		j = findfirst(possible_indices[i] && wordle_original_inds[i] for i in eachindex(possible_indices))
		results = [(word = nyt_valid_words[j], groupsize = 1, feedbacklist = [242])]
		put!(resultqueue, (k, results))
		return results
	end
	# guessinds = eachindex(nyt_valid_words)
	# guessinds = view(freqsortinds, 1:8000)
	# results = foldxt(vcat, guessinds |> Map(i -> eval_absurdle_group(i, possible_indices, feedback_allocations[i])))
	# feedback_counts = zeros(243)
	# @info "Evaluating $(length(guessinds)) possible guesses as a new calculation"
	# results = mapreduce(i -> eval_absurdle_group(i, possible_indices, feedback_counts), vcat, guessinds)
	# results = Vector{NamedTuple{(:word, :feedback, :groupsize), Tuple{String, Int64, Int64}}}()
	# for i in guessinds
		# append!(results, eval_absurdle_group(i, possible_indices, feedback_counts))
	# end
	# feedback_counts = zeros(length(nyt_valid_words), 243)
	results = eval_absurdle_groups(guessinds, possible_indices, feedback_counts)
	sort!(results, by = a -> a.groupsize)
	put!(resultqueue, (k, results))
	return results
end

# ╔═╡ 1e00f359-dce3-4102-bf5a-03a00da7a195
const feedback_sets = make_feedback_sets(feedback_matrix, savedata)

# ╔═╡ 8ce8a949-6269-4e0d-b515-f4b2f53be6a9
...
# ╔═╡ d0c71456-fafc-4228-8dd0-c4dbcb5f5045
function get_possible_indices!(gamesteps, outinds)
	outinds .= true
	isempty(gamesteps) && return baseinds
	length(gamesteps) == 1 && return get_possible_indices(first(gamesteps)...)
	for step in gamesteps
		outinds .*= get_possible_indices(step...)
	end
	return outinds
end
...
# ╔═╡ 7cb91a26-4be5-4e8e-8d8c-9ee596380b89
...
# ╔═╡ 9e6dd99a-0300-48ba-a296-b13aa761c1ea
function eval_absurdle_guesses_gamesteps(gamesteps, guessinds, feedback_counts) 
	# @info "Evaluating guesses for $gamesteps"
	# isempty(gamesteps) && return base_absurdle_guesses
	# answer_indices = first(index_allocations)
	# get_possible_indices!(gamesteps, answer_indices)
	answer_indices = get_possible_indices(gamesteps)
	# @info "number of possible answers is $(count(answer_indices))"
	put!(lookup_time_channel, ((answer_indices, guessinds), now()))
	eval_absurdle_guesses(answer_indices, guessinds, feedback_counts)
end

# ╔═╡ 64e019d7-40f8-4925-ae8b-b9440192721b
function solve_absurdle(gamesteps::Vector{Tuple{String, Int64}}; branchnum = 2, initbranch = 2, guessinds = freqsortinds[1:8000], feedback_counts = zeros(Int64, 243), usethreads=false)
	beststeps = fetch(leaststeps)
	if isempty(gamesteps)
		@info "Starting solver with initial branching of $initbranch and subsequent branching of $branchnum"
		put!(progresschannel, -1)
	end
	if length(gamesteps) > beststeps 
		# @info "On thread $(threadid()) ending analysis of game $(gamesteps) because it can no longer be the best result"
		return false
	end
	
	if length(gamesteps) == 1
		@info "Considering new game starting with guess: $(first(gamesteps)[1])"
	end
	guesses = eval_absurdle_guesses_gamesteps(gamesteps, guessinds, feedback_counts)

	if isempty(gamesteps)
		@info "Starting solver with initial guess list of $([a.word for a in view(guesses, 1:initbranch)])"
	end
	# @info "smallest groups guess is $(first(guesses).word) with feedback $(first(guesses).feedback) and group size $(first(guesses).groupsize)"
	if length(guesses) == 1 
		numsteps = length(gamesteps)
		put!(stepwatch, numsteps)
		beststeps = fetch(leaststeps)
		# @info "On thread $(threadid()).  Best step so far of $beststeps.  Game $gamesteps ended with final guess: $(first(guesses).word) and possible remaining words of $(nyt_valid_words[get_possible_indices(gamesteps)])"
		
		put!(solution_channel, (numsteps = numsteps, finalguess = first(guesses), gamesteps = gamesteps))
		return true
	end
	...
	smallestgroup = first(guesses).groupsize
	groupnum = findlast(a -> a.groupsize == smallestgroup, guesses)
	num = if smallestgroup == 1
		groupnum
	elseif isempty(gamesteps)
		max(groupnum, initbranch)
	else
		max(groupnum, branchnum)
	end
	
	guesslist = view(guesses, 1:num)

	if smallestgroup == 1 && groupnum > 1
		for guess in guesslist
			filter!(f -> f != 242, guess.feedbacklist)
		end
	end
	
	if isempty(gamesteps)
		put!(progresschannel, num)
	end

	if usethreads
		@threads for guess in guesslist
			# this way each call to solve_absurdle allocates its own feedback vector
			for feedback in guess.feedbacklist
				solve_absurdle(vcat(gamesteps, (guess.word, feedback)); branchnum=branchnum, guessinds=guessinds, usethreads = usethreads)
			end
		end
	else
		for guess in guesslist
			for feedback in guess.feedbacklist
				solve_absurdle(vcat(gamesteps, (guess.word, feedback)); branchnum=branchnum, guessinds=guessinds, feedback_counts = feedback_counts, usethreads=usethreads)
			end
		end
	end

	#after this loop has run for a game with one step that starting word is done being analyzed
	if length(gamesteps) == 1
		@info "Completed analysis of starting game $(first(gamesteps)).  Best solution so far has $beststeps steps"
		put!(progresschannel, 1)
	elseif isempty(gamesteps)
		@info "Done with solution using $branchnum branches and $initbranch initial branches"
		put!(progresschannel, 0)
	end
	return (branchnum, initbranch)
end

# ╔═╡ ba5850d1-7238-4c16-a6df-de1fc5b9bd63
#figure out how many groups of words there are given feedback and a probability weighting of answers
function eval_group(guess_index::Integer, possible_indices::AbstractVector, weights::Vector{Float64}; feedback_counts = zeros(Int64, 243), feedback_probabilities = zeros(243), hard_mode_indices = possible_indices)
	feedback_counts .= 0 
	...
	expected_value = 0.0
	entropy = 0.0
	expected_hard_mode_guesses = 0.0
...
begin
	...
	function eval_guesses(possible_indices::AbstractVector, weights::Vector{Float64}, hard_mode_indices::AbstractVector)
	...
	function eval_guesses(gamesteps; numcommon = lastindex(nyt_valid_words), weight_func = uniform_ncommon_weights, weights = weight_func(numcommon), undosteps = Vector{Tuple{String, Vector{Int64}}}()) 
...
function show_game_eval(wordlegame; sortname = :expected_value, minimum_win_probability = 0.0, reversesort = false, hardmode = false, kwargs...)
	wordlegame_eval = eval_guesses(zip(wordlegame.guesses, wordlegame.feedback); undosteps = wordlegame.undos, kwargs...)
	...
	rankedguesstbl = sort(filter(a -> a.win_probability >= minimum_win_probability, guesstbl), sortname, rev = reversesort)
	hardmode && filter!(a -> a.hard_mode_guess, rankedguesstbl)
	numanswers = size(wordlegame_eval.ranked_answers, 1)
	numlikelyanswers = count(a -> a.probability > 0, wordlegame_eval.ranked_answers)
	message = if numanswers > 1
	md"""
	...
	md"""
	Only one possible answer remains: $(first(tbl).answer)
	"""
	end
	(message, rankedguesstbl)
end
...
#game state is updated after an answer is submitted
singleguess
...
# ╔═╡ 6d0575a7-7c5d-4ed0-92d3-83393fc33bd2
@bind wordlegame WordleGame(;answer_index_list = make_pdf_answer_index(8000, 5000))

# ╔═╡ c091310d-0646-4508-a532-3a702d00c8b7
@bind playselect make_guesseval_selector("Playable Game")

# ╔═╡ 10d85c3f-208f-4e2c-8abb-3f1b95e34dc8
...
WordleGame(;answer_index_list=[word_index["guard"]])
...
# ╔═╡ 0996fb3a-c58a-4bce-8319-23ad78cdcc58
@bind inputselect confirm(make_guesseval_selector("Input Game"))

# ╔═╡ a656c08e-98ab-4999-914e-a6a7afc22715
show_game_eval(gameinput; inputselect...)

# ╔═╡ 916c36c8-6f0f-48da-89b6-b6d46f35c657
@bind branches confirm(PlutoUI.combine() do Child
	md"""
	Branching Factor: $(Child(:branchnum, NumberField(1:100, default = 2)))
	Initial Branching: $(Child(:initbranch, NumberField(1:100, default = 2)))
	"""
end)
...
begin
	branches
	md"""
	Click to run absurdle solver with branching factor of $(branches.branchnum) and initial branch of $(branches.initbranch)
	
	$(@bind runabsurdle CounterButton())
	"""
end

# ╔═╡ ab47f281-ac81-4516-b1c8-d099fb6248ad
if runabsurdle == 0
	md"""
	Solver has not ben run since submitting new parameters
	"""
elseif runabsurdle > 1
	md"""
	Change parameters to run solver again
	"""
else
	md"""
	#### Running solver...
...
if runabsurdle == 1
	solvebranches = solve_absurdle(Vector{Tuple{String, Int64}}(); usethreads=true, branches...)
	Base.GC.gc()
	#trigger saving new data
	@spawn save_absurdle_solutions($absurdle_solutions)
	@spawn save_absurdle_lookup(lookup=$absurdlelookup)
	push!(previousruns, branches)
	md"""
	#### Showing solutions for branching $(solvebranches[1]) and initial branching of $(solvebranches[2])
	"""
elseif isnothing(last(previousruns))
	md"""
	#### No solutions to show
	"""
else
	md"""
	#### Showing solutions for most recent run with branching $(last(previousruns).branchnum) and initial branching of $(last(previousruns).initbranch)
	"""
end

# ╔═╡ 47e97ea5-aef3-4c69-ad4b-d0921da9acdd
solutionlist = process_solutions(runabsurdle)

# ╔═╡ 5f480cff-a25e-4fa5-a1b9-e2f1b210bbe3
md"""
Starting Index: $(@bind indstart confirm(Slider(1:100, show_value=true)))

Max Number: $(@bind maxnum confirm(NumberField(1:100, default = 12)))
"""

# ╔═╡ 197d66ea-2185-4f92-a20c-ecd44972daad
if !isnothing(solutionlist)
	md"""
	Starting Word: $(@bind startingwords MultiCheckBox(unique([a.gamesteps[1][1] for a in solutionlist]), default = [first(solutionlist).gamesteps[1][1]], select_all=true))
	"""
else
	startingwords = nothing
end
...
function score_wordle_game(game; finalscore=true)
	answer = nyt_valid_words[game[3]]
	...
	if game[2][1] != ""
		...
		gameover = (guessfeedback == winfeedback) || (game[1] == game.numguesses-1)
		
		outcomeclass = if !finalscore
			""""""
		elseif guessfeedback == winfeedback
			"""
			setTimeout(()=>{}, 2500);
			board.classList.add("gamewon");
			"""
		elseif game[1] == game.numguesses - 1
			"""
			setTimeout(() => {}, 1500);
		...
		HTML("""
			<script>
				...
				$jsblock
				$outcomeclass
				$(gameover ? """board.classList.add("gameover");""" : """""")
			</script>
			<style>
...
# ╔═╡ a526bdc5-1929-4d11-9084-bd7cf0553dc4
function show_wordle_game(answer::AbstractString, guesses::AbstractVector{T}; kwargs...) where T <: AbstractString
	...
	show_wordle_game(gamelist; kwargs...)
end

# ╔═╡ 0738c0a1-d64c-494f-9918-8f581a92710e
function show_wordle_game(gamelist; sizepct::Integer = 100, show_overlay = true)
	guesses = [first(a) for a in gamelist]
	feedbacklist = [digits(last(a), base=3, pad = 5) for a in gamelist]
	...
	boardclass = show_overlay ? (gamewin ? "gamewin" : lastindex(guesses) == 6 ? "$answer gamelost" : "") : ""
	HTML("""
	<div class = "wordle-game-display">
	<div class = "wordle-game-grid $boardclass sizepct$sizepct">
		$feedbackboxes
		$winboxes
		$blankboxes
	</div>
	...
	<style>
		...
		.wordle-game-grid.sizepct$sizepct {
			...
			height: calc($(sizepct/100)*var(--container-width)*6/5);
		...
		.wordle-game-grid:hover.gamewon::after, .$answer:hover::after {
			content: '';
			position: absolute;
			margin: 0;
			padding: 0;
...
show_wordle_game("apple", [[sample_answer() for _ in 1:3]; "apple"])

# ╔═╡ 4552efbc-f3d5-43c1-8a34-8052ac060d07
show_wordle_game(sample_answer(), [guess_random_word() for _ in 1:6])

# ╔═╡ ec0a5cec-450d-4eee-9a89-ea4ec94980dd
if !isnothing(solutionlist) && runabsurdle == 1
	#if a new solver has been run then empty the display results and fill it with the new ones
	while !isempty(absurdledisplay)
		pop!(absurdledisplay)
	end
	for a in solutionlist
		push!(absurdledisplay, show_wordle_game([a.gamesteps; (a.finalguess.word, a.finalguess.feedbacklist |> first)], show_overlay=false, sizepct = 25))
	end
	newdisplay = true
else
	newdisplay = false
...
# ╔═╡ d816aed5-fee7-4c30-931d-1a207fd32285
displaysolutions(absurdledisplay, solutionlist, indstart, maxnum, newdisplay; startingwords = isnothing(startingwords) ? startingwords : Set(startingwords))
...
julia_version = "1.9.1"
manifest_format = "2.0"
project_hash = "056d55f4467a40757d7c150a7ac91c1e26c37e0c"
